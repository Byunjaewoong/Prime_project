...
const route = process.argv[3] ?? 'Vortex_GPU';
...
await page.goto(`http://localhost:3000/works/${route}`,{waitUntil:'networkidle'});
...
const pixels=await page.evaluate(route=>new Promise(resolve=>requestAnimationFrame(()=>{
 const canvas=document.querySelectorAll('canvas')[route==='Vortex'?1:0],gl=canvas.getContext('webgl2'),data=new Uint8Array(canvas.width*canvas.height*4);
...
await page.screenshot({path:`artifacts/browser-260914/mobile-${route}-touch.png`});
...
fs.writeFileSync(`artifacts/browser-260914/${route}-resources.json`,JSON.stringify(result,null,2));
console.log(result);
await browser.close();
