  ...
  for (const mobile of [false, true]) {
    ...
    await page.route('**/__snow/**', async route => {
      ...
      if (pathname.startsWith('/__snow/three/')) {
        ...
        const files = { '/__snow/App.js': 'app/works/Snow-walker/core/App.ts', '/__snow/FieldStyles.js': 'app/works/Snow-walker/core/FieldStyles.ts', '/__snow/CameraFilter.js': 'app/works/Snow-walker/core/CameraFilter.ts', '/__snow/disposeObject.js': 'app/lib/disposeObject.ts' };
        body = compile(files[pathname]).replace('"@/app/lib/disposeObject"', '"/__snow/disposeObject.js"').replace('"./FieldStyles"', '"./FieldStyles.js"').replace('"./CameraFilter"', '"./CameraFilter.js"');
      }
    ...
    assert.equal(filterToggles, true);
