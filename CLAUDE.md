...
Snow-walker core/Grain.ts applies Vortex_GPU's display treatment (haze, hash grain, vignette, S-curve, lifted blacks) to a captured camera image. Only green/gold fields use it. The Camera filter checkbox changes the existing App flag; it must not recreate App or affect walking state. Capture at the drawing-buffer resolution, convert the linear image to display color before the film treatment, and dispose both the separate filter scene and render target on cleanup.
