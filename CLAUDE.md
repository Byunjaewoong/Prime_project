...
Snow-walker: core/FieldStyles.ts defines snow/green/gold presets and a seeded, code-generated grass texture. Left click/tap cycles fields; canvas contextmenu plants a tree. Blend the cached ground material, texture samples, lighting, fog and footprint colors without resetting the player, mixer, path, footprints or existing trees. Keep the camera fixed. Dispose skeletons, the grass shader texture and renderer resources on unmount. Do not force context loss during effect cleanup: React Strict Mode reuses the same canvas for setup/cleanup/setup.
